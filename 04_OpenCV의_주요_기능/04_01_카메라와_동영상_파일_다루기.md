...
```
* <img src="./img/OCV001.PNG" /> <br/> apiPreference 인자에는 VideoCaptureAPIs 열거형 상수 중 하나를 지정 <br/> 대부분의 경우 apiPreference 인자를 생략하거나 기본값인 CAP_ANY를 지정하며, 이 경우 시스템이 알아서 적절한 방법을 선택하여 사용
```cpp
// 예를 들어 컴퓨터에 연결된 기본 카메라로부터 한 프레임의 정지 영상을 받아 오려면 다음과 같은 형태로 코드를 작성
VideoCapture cap(0);

Mat frame1, frame2;
cap >> frame1;       // frame1은 VideoCapture::operator >>() 연산자 재정의 함수를 사용하여 받음
cap.read(frame2);    // frame2는 VideoCapture::read() 함수를 사용하여 받음
```
* <img src="./img/OCV002.PNG" /> <br/> propId 인수는 속성 ID. VideoCaptureProperties 열거형 중 하나를 지정
```cpp
...
```
* <img src="./img/OCV003.PNG" /> <br/> VideoWriter 객체를 생성하였으면 VideoWriter::open() 멤버 함수를 이용하여 저장할 동영상 파일을 쓰기 모드로 열어야 함
* <img src="./img/OCV004.PNG" /> <br/> fourcc 코드와 관련 코덱 정보
```cpp
